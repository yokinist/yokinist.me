...
import { filterPublishedProjects, getAllPosts, getAllProjects, getAllProjectTags } from '~/lib/notion';
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const tags = getAllProjectTags({ posts });
...
export const getStaticPaths: GetStaticPaths = async () => {
  const posts = await getAllProjects({ includedPages: false });
  const tags = getAllProjectTags({ posts });
  return {
...
const TagPage: NextPage<Props> = ({ tags, posts, currentTag, post, blockMap, emailHash }) => {
  ...
  return (
    ...
    >
      ...
      <SearchLayout tags={tags} posts={posts} currentTag={currentTag} postType="project" />
    </Container>
  );
};
